cc = read.csv('OneDrive - Harvard University/Courses/Stat 236/EU-Voting-Patterns/simulated_data/country_country_adj.csv',
              row.names = 1)
...
# Loading this data for country names
...
ms.out$R = as.data.frame(ms.out$R)
ms.out$vertices = as.data.frame(ms.out$vertices)
ms.out$centers = as.data.frame(ms.out$centers)
colnames(ms.out$R) <- c("x", "y")
colnames(ms.out$vertices) <- c("x", "y")
colnames(ms.out$centers) <- c("x", "y")
ms.out$R$names = colnames(cc)

p = ggplot() +
      # Grey points for `ms.out$R`
      geom_point(data = data.frame(ms.out$R), aes(x = x, y = y), color = "grey", size = 2) +
      geom_text(data = ms.out$R, aes(x = x, y = y, label = names), hjust = -0.2, vjust = -0.5, size = 3, fontface = "bold") +
      
      # Dashed polygon for `ms.out$vertices`
      geom_polygon(data = data.frame(ms.out$vertices[c(1, 2, 3, 1), ]), aes(x = x, y = y), color = "black", linetype = "dashed", fill = NA, size = 1) +
      
      # Blue point for `ms.out$centers`
      geom_point(data = data.frame(ms.out$centers), aes(x = x, y = y), color = "blue", size = 4) +
      
      # Labels and theme adjustments
      labs(title = "First two ratios", fill = "Value (Opacity)", x = NULL, y = NULL) +
      theme_minimal() +
...
for(i in 1:(ncol(mship)-1)){
  ...
  p = ggplot(data = filtered_countries) +
    ...
    coord_sf(xlim = c(-15,35), ylim = c(30,75), expand = FALSE) +
    scale_x_continuous(
  ...
  plot_list[[i+1]] = p + theme(plot.margin = unit(c(0.1,0,0.1,0), "cm"))
}
